          ...
          {!isSubscribed ? (
            <div className="text-center py-8">
              <div className="mb-4 flex justify-center">
                <svg className="size-12 text-slate-300" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
                </svg>
              </div>
              <p className="text-slate-500 mb-4">Unlock book browsing with Pro</p>
              <Link
                href="/subscribe"
                className="inline-block px-6 py-3 bg-brand rounded-full font-medium text-white"
              >
                Upgrade to Pro
              </Link>
            </div>
            ...
            <div className="space-y-2">
              <button
                ...
                className={`w-full p-4 rounded-xl text-left transition-all ${
                  !selectedBook ? "bg-brand text-white" : "bg-slate-50 hover:bg-slate-100"
                }`}
              >
              ...
              {getAllBooks().slice(0, 50).map(({ book, author, count }) => (
                <button
                  ...
                  className={`w-full p-4 rounded-xl text-left transition-all ${
                    selectedBook === book ? "bg-brand text-white" : "bg-slate-50 hover:bg-slate-100"
                  }`}
                >
                  <span className="font-medium block">{book}</span>
