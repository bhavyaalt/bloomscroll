...
// Use WBTC on Starknet (wrapped BTC)
...
// Store for current wallet session
...
export async function connectWallet(): Promise<StarkZapWallet | null> {
  ...
  try {
    // Try Cartridge Controller first (best UX - no seed phrase)
...
export async function refreshBalances(): Promise<StarkZapWallet | null> {
  ...
  try {
    const { wallet } = await sdk.onboard({
      strategy: "cartridge",
      deploy: "never", // Just reconnect, don't deploy
    });
...
export async function claimPendingRewards(
  ...
  try {
    // Call server-side API to distribute rewards (keeps treasury key secure)
    ...
    // Clear pending after successful claim
    clearPendingRewards();
...
// Helper to convert string to Address type
